define(["mapTerrain", "loader"], function(MapTerrain, Loader) {
	var CanvasDebugger = function(map) {
			this.map = map;
	...
	CanvasDebugger.prototype.debug = function() {
	...
	CanvasDebugger.prototype.drawlines = function() {
	...
	CanvasDebugger.prototype.generateImageFromTerrain = function() {
		var context = this.map.canvas.getContext("2d");
		var matrix = this.map.mapTerrainMatrix;
		var sliceParams = {
		...
		for(var i = 0; i < matrix.length; i++) {
			...
			var x = iWidth % this.canvasWidth;
			var y = Math.floor(iWidth / this.canvasWidth) * this.barHeight;
			context.drawImage(Loader.SPRITESHEET, sliceParams.x, sliceParams.y, sliceParams.width, sliceParams.height, x, y, this.barWidth, this.barHeight);
		}
	};
	return CanvasDebugger;
});